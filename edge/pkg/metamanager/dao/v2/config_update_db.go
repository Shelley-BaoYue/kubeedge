...
var (
	MetaGVRConfigUpdate = schema.GroupVersionResource{
...
func (dao *ConfigUpdate) Get() (string, string, *operationv1alpha2.ConfigUpdateJobSpec, error) {
	row, err := onlyOneConfigUpdateRowByGVR(dao.db, MetaGVRConfigUpdate.String())
	if err != nil {
		return "", "", nil, err
	}
	if row == nil {
...
func (dao *ConfigUpdate) Save(jobname, nodename string, spec *operationv1alpha2.ConfigUpdateJobSpec) error {
	// Cleaning up historical configupdatejobtaskrequests datas
...
// onlyOneConfigUpdateRowByGVR returns the first row of data that query by GVR.
func onlyOneConfigUpdateRowByGVR(db orm.Ormer, gvr string) (*MetaV2, error) {
	var rows []MetaV2
